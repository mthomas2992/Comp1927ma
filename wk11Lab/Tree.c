...
typedef struct TreeRep {
	Link  root;
	Style insert;
	int   ncompares; //modfy these values
	int   nrotates; //these values
} TreeRep;
...
int cmp(Tree T,int k1, int k2){
	T->ncompares++;
	return k1 - k2;
}

int less(Tree T, int k1, int k2){
	return (cmp(T,k1,k2) < 0);
}

int eq(Tree T, int k1, int k2){
	return (cmp(T,k1,k2) == 0);
}
...
Link insert(Link t, Item it)
{
	...
	int diff = cmp(t->within,key(it),key(t->value));
	if (diff == 0)
		t->value = it;
	else if (diff < 0)
...
Link insertSplay(Link t, Item it)
{
	Key v = key(it);
	if (t == NULL) return newNode(it);
	int diff = cmp(t->within,v,key(t->value));
	if (diff == 0)
		t->value = it;
	else if (diff < 0) {
		if (t->left == NULL) {
		...
		if (less(t->within,v,key(t->left->value))) {
		...
		if (less(t->within,key(t->right->value),v)) {
...
static Link search(Link t, Key k)
{
	...
	Link res = NULL;
	int diff = cmp(t->within,k,t->value);
	if (diff == 0)
		res = t;
	else if (diff < 0)
		res = search(t->left, k);
...
static Link searchSplay(Link t, Key k, int *found)
{
	...
	else if (eq(t->within,key(t->value),k)) {
		*found = 1; // item found, store true
		res =  t;
	}
	else if (less(t->within,k,key(t->value))) {
		if (t->left == NULL){
			*found = 0;// item not found
			//res = rotateRight(t);
			res = t;
		}
		else if (eq(t->within,key(t->left->value),k)) {
			*found = 1;
			res = rotateR(t);
		}
		else {
			if (less(t->within,k,key(t->left->value))) {
				// left-left
		...
		else if (eq(t->within,key(t->right->value),k)) {
			*found = 1;
			res = rotateL(t);
		}
		else{
			if (less(t->within,key(t->right->value),k)) {
				/* right-right */
...
Link rotateR(Link n1) //TODO Add counter
{
...
Link rotateL(Link n2) //TODO Add counter
{
